...
if (sm == "") {
  ...
  let tocDiv = document.getElementById("catalog");
  ...
  book.ready.then(() => {
    ...
    let keyListener = function (e: { keyCode: any; which: any; }) {
  ...
  let recursionHandle = function (toc: NavItem[], doc: string[], i: number) {
  ...
  rendition.on("rendered", function (section: { href: string; }) {
    ...
    book.loaded.navigation.then(function (toc) {
      ...
      jQuery('.catalog-item').on('click', function () {
        ...
        let url = jQuery(this).attr("data-catalog");
        console.log(url);
